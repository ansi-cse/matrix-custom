...
class Client extends MatrixApi {
  ...
  String? get tokenRefresh => _refreshToken;

  set tokenRefresh(String? token) => _refreshToken = token;
  ...
  Future<void> _innerSync({Duration? timeout}) async {
    ...
    try {
      ...
      if (e.error == MatrixError.M_UNKNOWN_TOKEN) {
        //refreshToken
        RefreshResponse ref = await refresh(_refreshToken!);
        print('ref.accessToken ${ref.accessToken}');
        print('ref.refreshToken ${ref.refreshToken}');
        if (ref.accessToken != null && ref.refreshToken != null) {
          accessToken = ref.accessToken;
          _refreshToken = ref.refreshToken;
          _sync();
        } else {
